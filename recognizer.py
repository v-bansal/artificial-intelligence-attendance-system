...
def db_timeout(id):
    ...
    mycursor.execute(sql, val)
    mydb.commit()
    print("Timeout Marked")


start=time.time()
period=8
face_cas = cv2.CascadeClassifier("trainer/haarcascade_frontalface_default.xml")
    ...
    for (x,y,w,h) in faces:
        ...
        if(conf < 50):
            if((str(id)) not in dict):
                dict[str(id)]=str(id)
                db_timein(id)
                playsound('sound/thank_you.mp3')
                
            else :
                playsound('sound/youarepresent.mp3')
                db_timeout(id)
